pipeline {
    ...
    post {
        success {
            ...
            withCredentials([usernamePassword(credentialsId: 'tomcat_server', usernameVariable: 'deployer', passwordVariable: 'deployer')]) {
                sh '''
                    ...
                    if [ -f "$WAR_FILE" ]; then
                        ...
                        curl --upload-file "$WAR_FILE" "http://13.232.180.182:8080/manager/text/deploy?path=/store&update=true" \
