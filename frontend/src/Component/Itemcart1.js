...
const{name}=useContext(namecontext);
  ...
  const placeOrderHandler=()=>{
   ...
   console.log("name is",name);
    ...
    {a && axios.post("http://localhost:8082/getbill",bill).then(response=>{
              ...
              {cart.map((item, index) => {
                return (
                  <tr key={index}>
                    ...
                    <td>{item.price}</td>
                    <td>
                        <button onClick={()=>handleChange(item,+1)} className="btn btn-info ms-2">+</button>
                    
                         <button  className="btn btn-primary ms-2" >{item.amount}</button>
                         
                        <button onClick={()=>handleChange(item,-1)} className="btn btn-info ms-2">-</button>

                        <button onClick={()=>handleRemove(item.id)}  className="btn btn-danger ms-2">Remove Item</button>
                    </td>
                  </tr>
                );
              })}
